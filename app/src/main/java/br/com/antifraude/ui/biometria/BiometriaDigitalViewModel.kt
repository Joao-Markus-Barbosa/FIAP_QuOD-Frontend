...
class BiometriaDigitalViewModel : ViewModel() {

    private val _resultado = MutableLiveData<String>()
    val resultado: LiveData<String> = _resultado

    fun validarBiometria(request: BiometriaDigitalRequest) {
        viewModelScope.launch {
            try {
                val response = RetrofitInstance.api.validarBiometriaDigital(request)
                ...
                if (response.isSuccessful && !body.isNullOrBlank()) {
                    ...
                    _resultado.postValue("❌ ${body ?: "Erro na validação digital."}")
                }
